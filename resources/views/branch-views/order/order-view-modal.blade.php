                            ...
                            <div class="col-sm-6">
                                <div class="text-sm-right">
                                    <div class="d-flex flex-wrap gap-2 justify-content-sm-end">
                                        ...
                                        @endif
                                        <a class="btn btn-info" onclick="print_invoice_order_view_modal({{$order['id']}})" href="javascript:void(0);">
                                        ...
                                        <div class="media gap-3 w-max-content">
                                            ...
                                            <div class="media-body text-dark fz-12">
                                                ...
                                                <div class="d-flex gap-2">
                                                    ...
                                                    @if(isset($variation_det[0]['price']))
                                                        @if($variation_det)
                                                            <span>
...
{{--                                    <dd class="col-6 text-dark text-right">{{ \App\CentralLogics\Helpers::set_symbol($add_ons_cost) }}</dd>--}}
                                    ...
                                    <dd class="col-6 text-dark text-right">{{ \App\CentralLogics\Helpers::set_symbol($sub_total =$sub_total-$total_dis_on_pro) }}</dd>
...
{{--                            @if($order['order_type'] != 'pos')--}}
                                <div class="">
                                    ...
                                    <select name="order_status" onchange="route_alert('{{route('branch.orders.status',['id'=>$order['id']])}}'+'&order_status='+ this.value,'{{\App\CentralLogics\translate("Change the order status to ") }}'+  this.value)" class="status custom-select">
...
{{--                            @endif--}}
                            @if($order->customer && $order['order_type'] != 'pos')
                                <div class="">
                                    {{--                                need change option--}}
                                            ...
                                            @if(isset($origin))
                                                <a target="_blank" class="text-dark"
                                                   ...
                                                   href="http://maps.google.com/maps?z=12&t=m&q=loc:{{$current['latitude']}}+{{$current['longitude']}}">
                                                    <img width="13" src="{{asset('public-assets/assets/admin/img/icons/location.png')}}" alt="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {{$origin['location']?? ''}}
                                                </a>
                                            @else
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="delivery--information-single flex-column">
                                            ...
                                            <div class="d-flex">
                                                ...
                                                <div class="info">{{ $address ? @$address['floor'] : ''}}</div>
                                            </div>
                                            <div class="d-flex">
                                                <div class="name">{{ translate('house') }}</div>
                                                <div class="info">{{ $address ? @$address['house'] : ''}}</div>
                                            </div>
                                            <div class="d-flex">
                                                <div class="name">{{ translate('road') }}</div>
                                                <div class="info">{{ $address ? @$address['road'] : ''}}</div>
                                            </div>
                                            <hr class="w-100">
...
{{--                @endif--}}
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <div class="media flex-wrap gap-3">
                                ...
                                <div class="media-body d-flex flex-column gap-1">
                                    <a class="text-dark"><strong>{{$order->customer['f_name'].' '.$order->customer['l_name']}}</strong></a>
                                    <span class="text-dark"> <strong>{{$order->customer['orders_count']}}</strong> {{translate('Orders')}}</span>
                    ...
                    <ul class="list-group">
                        @foreach(\App\Model\DeliveryMan::where(['is_active'=> 1])->whereIn('branch_id', [0, auth('branch')->id()])->get() as $deliveryMan)
                            <li class="list-group-item d-flex flex-wrap align-items-center gap-3 justify-content-between">
                                ...
                                <a data-order-id="{{$order['id']}}" data-delivery-man-id="{{$deliveryMan->id}}" class="btn btn-primary btn-sm assign-delivery-man-modal-btn">{{translate('Assign')}}</a>
                            </li>
                        @endforeach
    ...
    <div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="deliveryInfoModal" id="deliveryInfoModal"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content border-primary">
                ...
                @if(isset($address))
                    <form action="{{route('branch.order.update-shipping',[@$address['id']])}}" method="post">
                        @csrf
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="address_type" class="form-control"
                                       placeholder="{{translate('EX : Home')}}" value="{{ $address ? @$address['address_type'] : '' }}" required>
                            </div>
                            <div class="form-group">
                                <label>{{translate('Name')}}</label>
                                <input type="text" class="form-control" name="contact_person_name"
                                       placeholder="{{translate('EX : Jhon Doe')}}" value="{{ $address ? @$address['contact_person_name'] : '' }}" required>
                            </div>
                            <div class="form-group">
                                <label>{{translate('Contact_Number')}}</label>
                                <input type="text" class="form-control" name="contact_person_number"
                                       placeholder="{{translate('EX : 01888888888')}}" value="{{ $address ? @$address['contact_person_number'] : '' }}" required>
                            </div>
                            <div class="form-group">
                                <label>{{translate('floor')}}</label>
                                <input type="text" class="form-control" name="floor"
                                       placeholder="{{translate('EX : 5')}}" value="{{ $address ? @$address['floor'] : '' }}" >
                            </div>
                            <div class="form-group">
                                <label>{{translate('house')}}</label>
                                <input type="text" class="form-control" name="house"
                                       placeholder="{{translate('EX : 21/B')}}" value="{{ $address ? @$address['house'] : '' }}" >
                            </div>
                            <div class="form-group">
                                <label>{{translate('road')}}</label>
                                <input type="text" class="form-control" name="road"
                                       placeholder="{{translate('EX : Baker Street')}}" value="{{ $address ? @$address['road'] : '' }}" >
                            </div>
                            <div class="form-group">
                                <label>{{translate('Address')}}</label>
                                <input type="text" class="form-control" name="address"
                                       placeholder="{{translate('EX : Dhaka,_Bangladesh')}}" value="{{ $address ? @$address['address'] : '' }}" required>
                            </div>
                            <div class="form-group">
                                <label>{{translate('latitude')}}</label>
                                <input type="text" class="form-control" name="latitude"
                                       placeholder="{{translate('EX : 23.796584198263794')}}" value="{{ $address ? @$address['latitude'] : '' }}" required>
                            </div>
                            <div class="form-group">
                                <label>{{translate('longitude')}}</label>
                                <input type="text" class="form-control" name="longitude"
                                       placeholder="{{translate('EX : 23.796584198263794')}}" value="{{ $address ? @$address['longitude'] : '' }}" required>
                            </div>
    ...
    <script>
        ...
        function print_invoice_order_view_modal(order_id) {
            $.get({
