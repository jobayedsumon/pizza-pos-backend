...
{{--                <dd class="col-6">{{$sub_total+$total_tax+$add_ons_cost." ".\App\CentralLogics\Helpers::currency_symbol()}}</dd>--}}
                @php($subtotal = $add_ons_cost + $item_price + $total_tax)
                ...
                <dt class="col-5" style="font-size: 20px">{{translate('Total')}}:</dt>
                <dd class="col-7" style="font-size: 20px">{{ \App\CentralLogics\Helpers::set_symbol($subtotal-$order['coupon_discount_amount']-$order['extra_discount']) }}</dd>
            </dl>
        </div>
    </div>
